...
async def to_code(config):
    ...
    if config[CONF_DAC_TYPE] == "internal":
        ...
        cg.add(var.set_mclk_pin(250 if CONF_I2S_MCLK_PIN not in config else config[CONF_I2S_MCLK_PIN]))
        cg.add(var.set_lrclk_pin(18 if CONF_I2S_LRCLK_PIN not in config else config[CONF_I2S_LRCLK_PIN]))
        cg.add(var.set_bclk_pin(0 if CONF_I2S_BCLK_PIN not in config else config[CONF_I2S_BCLK_PIN]))
        cg.add(var.set_dout_pin(1 if CONF_I2S_DOUT_PIN not in config else config[CONF_I2S_DOUT_PIN]))
